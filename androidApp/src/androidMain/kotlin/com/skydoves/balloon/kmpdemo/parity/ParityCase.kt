...
data class ParityCase(
  ...
  /** `0` leaves `setOverlayPaddingColor` unset; anything else paints the padding band. */
  val overlayPaddingColorArgb: Int = 0,
)
...
val PARITY_CASES: List<ParityCase> = buildList {
  ...
  // `setOverlayPaddingColor` paints the band the padding opened up. A `Circle` carrying an
  // explicit radius ignores the rect it is handed, so clearing the anchor shape at that same
  // radius used to erase the band completely and render nothing at all.
  add(
    ParityCase(
      id = "overlay-ring-oval",
      overlayShape = "OVAL",
      overlayPaddingDp = 12f,
      overlayPaddingColorArgb = PARITY_BORDER_COLOR,
    ),
  )
  add(
    ParityCase(
      id = "overlay-ring-circle",
      overlayShape = "CIRCLE",
      overlayRadiusDp = 40f,
      overlayPaddingDp = 12f,
      overlayPaddingColorArgb = PARITY_BORDER_COLOR,
