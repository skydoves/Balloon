...
// The baseline profile lives in `src/main/baselineProfiles/baseline-prof.txt`, which is the
// path AGP packages into the AAR (`main` is not remapped to `androidMain` here, so the Android
// source set really is `src/main`). It is generated by `:benchmark` against `:androidApp` and
// then filtered down to `com.skydoves.balloon.**`; see that module for how to regenerate it.
android {
